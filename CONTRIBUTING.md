...
- Issues are labeled by phase (`phase:astral-council`, `phase:lunar-core`, etc.) and by track (`track:A`, `track:B`).
...
- Nothing fits? Open an issue proposing what you'd like to build. Proposing a new Astral Council agent has its own issue template.
...
- **Track A:** unit tests per module. Any change to `council/`'s agents or consensus logic should run against the evaluation set in `research/` before merging — this protects against silently regressing consensus quality, which is the single most important property of the whole Council concept.
