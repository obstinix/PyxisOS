...
The repo currently has only a LICENSE file (Apache-2.0 — good, that decision's already made). Suggested structure:
...
├── native/                     ← Track B workspace: Lunar Core, Aegis, native Nebula
│   ├── lunar-core/
│   ├── aegis/
│   └── nebula/
...
└── .github/
    ├── ISSUE_TEMPLATE/
    └── workflows/               ← CI (lint/test now; QEMU boot tests once native/ has content)
```
...
- Given `native/` will eventually need very different CI (QEMU boot tests, kernel-specific tooling) than `council/`/`shell/` (standard app CI), keep them decoupled enough to split into a separate repo later without much pain — don't let them share build tooling more than necessary.
- Add a `CONTRIBUTING.md` early, since the "AI layer vs. kernel layer" split means you're recruiting two different kinds of contributors with different onboarding needs.
...
- **Requirements:** 3D scene graph for windows · GPU-accelerated rendering · physics-based window interaction (inertia, snapping) · gesture/controller navigation · OpenXR support · multiple fast-switching spatial workspaces.
- **Acceptance criteria:** a demoable spatial prototype holding a comfortable frame rate for VR (commonly cited comfort thresholds start around 72fps+) with multiple floating windows.
- **Tech notes:** prototype on an existing engine (Godot, Bevy, Unity, or Three.js/WebXR) rather than a custom renderer — real-time 3D engines are themselves decade-scale efforts, and the novelty here is the desktop metaphor, not the renderer.
- **Dependencies:** benefits from Stellar Canvas's design system; can run entirely on Track A if built on an existing engine.
- **Effort:** an initial 3D window prototype is **weeks to ~2 months** on a game-engine base; a comfortable, daily-usable VR desktop is a long, iterative UX research effort.
- **Risks:** VR comfort/motion sickness is a genuinely hard UX problem — treat as an opt-in experimental mode until validated, not core UX.
